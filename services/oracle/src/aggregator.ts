...
export function rejectOutliers(prices: number[]): number[] {
  ...
  if (stdDev === 0) return prices; // all identical — nothing to reject
 ...
 */
export function computeIndexVwap(
  ...
  const price = constituents.reduce(
    (s, c) => s + c.price * (c.staticWeight / totalWeight),
    0,
  );
